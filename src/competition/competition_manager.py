...
class CompetitionManager:
    ...
    def list_scenarios(self) -> List[Dict[str, Any]]:
        """List available competition scenarios. Alias for list_available_scenarios."""
        scenarios = self.competition_service.list_scenarios()
        ...
        return scenario_data if scenarios else []
    ...
    def save_attempt(self, arg1=None, scenario_id=None, 
                    results=None, is_official=None):
        """
        Save a simulation attempt explicitly.
        
        Args:
            arg1: Can be player_id (str) or results (dict)
            scenario_id: Scenario ID (uses current if None)
            results: Simulation results (required if not provided as arg1)
            is_official: Whether this is an official attempt (uses current mode if None)
            
        Returns:
            Success status or attempt ID
        """
        # Handle the case where results is passed as the first positional argument
        # This is a common usage: attempt_id = competition.save_attempt(results)
        player_id = None
        if arg1 is not None:
            if isinstance(arg1, dict):
                # First argument is results
                results = arg1
            else:
                # First argument is player_id
                player_id = arg1
    ...
    def export_best_result(self, output_file: str) -> bool:
        """
        Export the best result for the current player and scenario to a file.
        
        Args:
            output_file: Path to save the exported result
        ...
        player_id = self.current_player_id
        if not player_id:
            raise ValueError("No player currently set")
        ...
        best_attempt = self.competition_service.get_best_attempt(
            player_id=player_id,
            scenario_id=scenario_id,
            is_official=True
        )
        
        if not best_attempt or not best_attempt.results:
            print(f"No results found for player {player_id} and scenario {scenario_id}")
            return False
            
        # Create export data
        export_data = {
            "player_id": best_attempt.player_id,
            "player_name": self.current_player_name,
            "scenario_id": best_attempt.scenario_id,
            "timestamp": best_attempt.timestamp.isoformat(),
            "is_official": best_attempt.is_official,
            "score": best_attempt.results.final_score,
            "components": {
                "population_survived": best_attempt.results.population_survived,
                "gdp_preserved": best_attempt.results.gdp_preserved,
                "infection_control": best_attempt.results.infection_control,
                "resource_efficiency": best_attempt.results.resource_efficiency,
                "time_to_containment": best_attempt.results.time_to_containment
            },
            "raw_metrics": best_attempt.results.raw_metrics if hasattr(best_attempt.results, "raw_metrics") else {}
        }
        
        try:
            with open(output_file, 'w') as f:
                json.dump(export_data, f, indent=2)
            print(f"Best result exported to: {output_file}")
            return True
        except Exception as e:
            print(f"Error exporting result: {e}")
            return False
            
    def get_detailed_metrics(self, attempt_id: str = None) -> List[Dict[str, Any]]:
        """
        Get detailed metrics from an attempt for visualization and analysis.
        
        Args:
            attempt_id: ID of the attempt (uses latest attempt if None)
            
        Returns:
            List of metrics data points, with each point being a dictionary of metrics at a time step
        """
        player_id = self.current_player_id
        if not player_id:
            raise ValueError("No player currently set")
            
        # Get all attempts for the current player and scenario
        attempts = self.competition_service.get_player_attempts(
            player_id=player_id,
            scenario_id=self.current_scenario_id
        )
        ...
        # If no specific attempt_id is provided, use the latest attempt
        target_attempt = None
        if attempt_id:
            target_attempt = next((a for a in attempts if a.id == attempt_id), None)
        else:
            # Sort by timestamp and get the most recent
            target_attempt = sorted(attempts, key=lambda a: a.timestamp, reverse=True)[0]
            
        if not target_attempt or not target_attempt.results or not hasattr(target_attempt.results, "raw_metrics"):
            print(f"No detailed metrics found for the selected attempt")
            return []
            
        # Get the raw metrics from the attempt
        raw_metrics = target_attempt.results.raw_metrics
        
        # Check if we have metrics_history in the raw metrics
        if "metrics_history" not in raw_metrics:
            print("No time-series metrics found in this attempt")
            return []
            
        metrics_history = raw_metrics["metrics_history"]
        
        # Process the metrics history into a format suitable for visualization
        processed_metrics = []
        for step, metrics in enumerate(metrics_history):
            # Create a base entry with step number
            entry = {"step": step}
            
            # Extract key metrics if they exist
            if "population" in metrics:
                pop = metrics["population"]
                total_population = pop.get("total", 1)  # Avoid division by zero
                entry["infected"] = pop.get("infected", 0)
                entry["infection_rate"] = entry["infected"] / total_population if total_population > 0 else 0
                entry["dead"] = pop.get("dead", 0)
                entry["survived"] = total_population - entry["dead"]
                
            if "economy" in metrics:
                econ = metrics["economy"]
                entry["gdp"] = econ.get("current_gdp", 0)
                entry["initial_gdp"] = econ.get("initial_gdp", 1)
                entry["gdp_ratio"] = entry["gdp"] / entry["initial_gdp"] if entry["initial_gdp"] > 0 else 0
                
            if "resources" in metrics:
                res = metrics["resources"]
                entry["available_resources"] = res.get("available", 0)
                entry["total_spent"] = res.get("total_spent", 0)
                
            if "resource_allocation" in metrics:
                alloc = metrics["resource_allocation"]
                entry["healthcare_resources"] = alloc.get("healthcare", 0)
                entry["economic_resources"] = alloc.get("economic", 0)
                entry["research_resources"] = alloc.get("research", 0)
                
            if "interventions" in metrics:
                interv = metrics["interventions"]
                entry["lockdown_level"] = interv.get("lockdown_level", 0)
                entry["travel_restricted"] = interv.get("travel_restricted", False)
